...
begin
  ...
  try
    ...
    Seek(SourceFile, FileSize(SourceFile) - iOffSet );

    BlockRead(SourceFile, Mark, 15 );
    if Mark.Mark = 'LYRICS200' then begin

       FLyrics2Size := StrToIntDef( Mark.Size, 0 );
       if FLyrics2Size > 0 then begin
          Seek( SourceFile, FileSize(SourceFile) - ( FLyrics2Size + iOffSet ) );
          SetLength( aBuff, 11 ); // LYRICSBEGIN
          BlockRead(SourceFile, aBuff[0], 11 );
...
var
  ...
  begin
    if Trim( sValue ) <> '' then begin
       ...
       BlockWrite( SourceFile, sTmp[1], Length( sTmp ) );
    end;
  end;
...
begin
  ...
  try
    ...
    if bTAG_PreserveDate then iFileAge := FileAge( FileName );
    ...
    if bUseLYR2 and ( bTAG_UseLYRICS3 or ( (Lyrics <> '') or (Writer <> '') or ( FIMG <> '' ) ) ) then begin
       ...
       if (Lyrics <> '') or (Writer <> '') or
          ...
          iFilePos := Filepos( SourceFile );

           BlockWrite( SourceFile, 'LYRICSBEGIN', 11 );
           if Lyrics <> '' then begin
              BlockWrite( SourceFile, 'IND0000210', 10 );
           end else begin
              BlockWrite( SourceFile, 'IND0000200', 10 );
           end;
           ...
           iFilepos := Filepos( SourceFile ) - iFilePos;
    ...
    BlockWrite(SourceFile, Tag, SizeOf( Tag ) );

    CloseFile(SourceFile);

    if bTAG_PreserveDate then FileSetDate( FileName, iFileAge );
  ...
  try
    ...
    if FExists or FExists2 then begin
       ...
       if FExists2 then begin
          Seek(SourceFile, FileSize(SourceFile) - Lyrics2Size );
          Truncate(SourceFile);
          FExists2 := false;
       end;
